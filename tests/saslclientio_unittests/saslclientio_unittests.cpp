...
static void* saved_on_sasl_frame_received_callback_context;
...
public:
	...
	MOCK_STATIC_METHOD_4(, SASL_FRAME_CODEC_HANDLE, sasl_frame_codec_create, FRAME_CODEC_HANDLE, frame_codec, ON_SASL_FRAME_RECEIVED, on_sasl_frame_received, ON_SASL_FRAME_CODEC_ERROR, on_sasl_frame_codec_error, void*, callback_context)
		...
		saved_on_sasl_frame_received_callback_context = callback_context;
	...
	/* io mocks */
	MOCK_STATIC_METHOD_4(, int, xio_open, XIO_HANDLE, xio, ON_BYTES_RECEIVED, on_bytes_received, ON_IO_STATE_CHANGED, on_io_state_changed, void*, callback_context)
	...
	MOCK_VOID_METHOD_END();
	MOCK_STATIC_METHOD_3(, void, test_on_io_state_changed, void*, context, IO_STATE, new_io_state, IO_STATE, previous_io_state)
	MOCK_VOID_METHOD_END();
...
extern "C"
{
	...
	DECLARE_GLOBAL_MOCK_METHOD_4(saslclientio_mocks, , int, xio_open, XIO_HANDLE, xio, ON_BYTES_RECEIVED, on_bytes_received, ON_IO_STATE_CHANGED, on_io_state_changed, void*, callback_context);
	DECLARE_GLOBAL_MOCK_METHOD_1(saslclientio_mocks, , int, xio_close, XIO_HANDLE, xio);
	...
	DECLARE_GLOBAL_MOCK_METHOD_3(saslclientio_mocks, , void, test_on_io_state_changed, void*, context, IO_STATE, new_io_state, IO_STATE, previous_io_state);
...
TEST_FUNCTION(saslclientio_open_with_valid_args_succeeds)
{
	...
	// act
	int result = saslclientio_open(sasl_io, test_on_bytes_received, test_on_io_state_changed, test_context);
...
static void setup_successful_sasl_handshake(amqp_definitions_mocks* definitions_mocks, CONCRETE_IO_HANDLE sasl_io)
{
...
TEST_FUNCTION(saslclientio_close_when_the_io_state_is_OPEN_closes_the_underlying_io)
{
	...
	(void)saslclientio_open(sasl_io, test_on_bytes_received, test_on_io_state_changed, test_context);
	setup_successful_sasl_handshake(&definitions_mocks, sasl_io);
	definitions_mocks.ResetAllCalls();
	mocks.ResetAllCalls();
...
TEST_FUNCTION(when_io_state_is_open_and_bytes_are_received_and_context_passed_to_open_was_NULL_NULL_is_passed_as_context_to_the_on_bytes_received_call)
{
	...
	CONCRETE_IO_HANDLE sasl_io = saslclientio_create(&saslclientio_config, NULL);
	unsigned char test_bytes[] = { 0x42, 0x43 };
	(void)saslclientio_open(sasl_io, test_on_bytes_received, test_on_io_state_changed, NULL);
	setup_successful_sasl_handshake(&definitions_mocks, sasl_io);
	mocks.ResetAllCalls();
	definitions_mocks.ResetAllCalls();
...
static void SetupSuccessfulChallengeResponse(saslclientio_mocks* mocks, amqp_definitions_mocks* definitions_mocks)
{
...
TEST_FUNCTION(when_a_SASL_challenge_is_received_after_the_mechanisms_the_sasl_mechanism_challenge_processing_is_invoked)
{
	...
	SetupSuccessfulChallengeResponse(&mocks, &definitions_mocks);
...
TEST_FUNCTION(when_the_mechanisms_array_does_not_contain_a_usable_SASL_mechanism_then_the_state_is_set_to_ERROR)
{
	...
	STRICT_EXPECTED_CALL(mocks, amqpvalue_get_symbol(test_sasl_server_mechanism, &test_sasl_server_mechanism_name));
	STRICT_EXPECTED_CALL(definitions_mocks, sasl_mechanisms_destroy(test_sasl_mechanisms_handle));
...
TEST_FUNCTION(when_encoded_bytes_are_received_they_are_given_to_xio_send)
{
	...
	SetupSendInit(&mocks, &definitions_mocks);
	saved_on_sasl_frame_received(saved_on_sasl_frame_received_callback_context, test_sasl_value);
